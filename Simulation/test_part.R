...
name_suf <- paste('_', comp[1],'_',cut_pct,'_',args[3],'_',arrID, sep = '')
...
# rmsep <- function(pre, tr){
#   sqrt(c(crossprod(summ$res))/length(tr))
# }
...
saveRDS(results, file = paste('./results/outpz1', name_suf, '.rds', sep = ''))
###################################

# Z2 (copy paste and replace all z1 with z2)
#######################################################################
# performance in training
...
saveRDS(results, file = paste('./results/outpz2', name_suf, '.rds', sep = ''))
