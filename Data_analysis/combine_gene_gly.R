...
######################################################
# Independent samples and original glycan peaks 
# gene <- readRDS('GeneDat.Rds')
# glycan <- readRDS('Glycan.Rda')
# id <- read.table('id_A_decpl.txt', header = T)
# 
# # independent samples in A
# gly <- glycan[rownames(glycan) %in% id$IID,]
# gene <- gene[rownames(gene) %in% id$IID,]
# # check
# dim(gly); dim(gene)
# 
# gly <- gly[order(rownames(gly)),]
...
# gene %<>% scale(scale = F)
# 
# save(gene, gly, file = 'Intl_gene_gly_A.RData')

######################################################
# # Test in training samples, not the gene data here is summarized from all samples
# gene <- readRDS('GeneDat.Rds')
# gly <- readRDS('gly_quantile_training.rds')
# 
# # independent samples in A
# gene <- gene[match(rownames(gly), rownames(gene)),]
# # check
# dim(gly); dim(gene)
# 
# all.equal(rownames(gly), rownames(gene))
...
# gene %<>% scale(scale = F)
# 
# save(gene, gly, file = 'Intl_gene_gly_quantile_test.RData')

######################################################
# Test in training samples, not the gene data here is summarized from all samples
gene <- readRDS('dat_genotyped_gene.rds')
gene <- readRDS('dat_pruned_gene.rds')

gly <- readRDS('Glycan.Rda')

# match glycan samples
...
save(gene, gly, file = 'Intl_gene_gly_pruned.RData')
save(gene, gly, file = 'Intl_gene_gly_genotyped.RData')